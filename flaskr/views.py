...
from flask import (
    Blueprint, abort, request, render_template, request,
    redirect, url_for, flash, session, jsonify, Markup
)
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        # メールに飛ばすほうがいい
        flash(
            Markup(f'<a href="{request.host_url}reset_password/{token}">パスワード設定用URL</a>'))
...
def user_search():
    ...
    if user_name:
        ...
        next_url = url_for('app.user_search', page=posts.next_num,
                           username=user_name) if posts.has_next else None
        prev_url = url_for('app.user_search', page=posts.prev_num,
                           username=user_name) if posts.has_prev else None
        users = posts.items
        # 検索結果のユーザを取得
        # UserテーブルとUserConnectテーブルを紐付けて、UserConnectテーブルのstatusを見る
        # from_user_id = 自分のID, to_user_id = 相手のID,
        # status=1の場合は、自分から友達申請中
        # to_user_id = 自分のID, from_user_id = 相手のID,
        # status=1の場合は、相手から友達申請中
...
def connect_user():
    ...
    if request.method == 'POST' and form.validate():
        if form.connect_condition.data == 'connect':
            new_connect = UserConnect(
                current_user.get_id(), form.to_user_id.data)
            with db.session.begin(subtransactions=True):
...
def message(id):
    ...
    read_message_ids = [
        message.id for message in messages if (not message.is_read) and
        (message.from_user_id == int(id))
    ]
    # すでに読まれていて、かつチェックしていない自分のメッセージをチェック
    not_checked_message_ids = [message.id for message in messages if message.is_read and (
        not message.is_checked) and (message.from_user_id == int(current_user.get_id()))]
    if not_checked_message_ids:
        with db.session(subtransactions=True):
